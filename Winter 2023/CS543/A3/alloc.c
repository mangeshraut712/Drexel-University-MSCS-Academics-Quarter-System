...
3,
{
	{ "main",  0, 	32*1024*1024, 31,  512*1024, 0, 31*1024*1024, (ulong)main_slabs},
...
if(q != nil) {
	...
	if(p->monitor)
		MM(p->pnum, pc, (ulong)B2D(q), size);
	print("served block of size %d within dopoolalloc...\n", q->size);
	return B2D(q);
}
...
void*
slabadd(Pool *p, ulong asize, ulong pc)
{
Bhdr *q, *list;
int size,index;
...
q = nil;
	...
	while (size < asize + sizeof(Bhdr) && size < 4096) {
  		size <<= 1;
}

index = slabsize(size) - 1 ;
lock(&p->l);
list = p->slabs[index];
...
if(list!= nil)
{
	...
	p->nalloc++;
	q->magic = MAGIC_A;
	p->slabs[index]= list->fwd;
	q->parent = nil;
	unlock(&p->l);
	return B2D(q);
}
unlock(&p->l);
return dopoolalloc(p, asize, pc);
}

void
slabdel(Pool *p, void *v)
{
Bhdr *t, *list;
ulong asize;
int  index;
...
else
{
	...
	list->parent = t;
  	...
  	p->slabs[index] = list->parent;
  	unlock(&p->l);
}

int
slabsize(ulong asize)
{
switch(asize){
...
if(p->cursize > p->ressize && (prog = currun()) != nil && prog->flags&Prestricted)
	return nil;
if(asize <= 4096)
	return slabadd(p, asize, getcallerpc(&p));
else
{
