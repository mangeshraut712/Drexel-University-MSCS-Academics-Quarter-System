...
disinit(void *a)
{
	...
	if(vflag){
		print("Initial Dis: \"%s\"\n", initmod);
	        print("Host owner: \"%s\"\n", eve);
		print("Build date: %s\n",__DATE__);
	        print("Kernel built on: %d\n", kerndate);
		hours = (kerndate / 3600) % 24;
		minutes = (kerndate / 60) % 60;
	    	seconds = kerndate % 60;
	    	print("Build time in hours, minutes and seconds: %02d:%02d:%02d\n", hours, minutes, seconds);
	}
